...
class SlimCryptDB {
    ...
    async _initializeDatabase() {
        try {
            ...
            await fs.mkdir(path.join(this.databaseDir, 'indexes'), { recursive: true });
    ...
    async _flushWAL() {
        ...
        const walData = this.walBuffer.map(entry => JSON.stringify(entry)).join('\n') + '\n';
    ...
    _validateDataAgainstSchema(data, schema) {
        ...
        if (schema.type && typeof data !== schema.type) {
            throw new Error(`Type mismatch: expected ${schema.type}, got ${typeof data}: ${JSON.stringify(data)}`);
        }
    ...
    async createIndex(tableName, indexName, columns, options = {}) {
        ...
        for (const item of data) {
            ...
            if (isUnique && index.has(indexKey)) {
                throw new Error(`Duplicate key violation for unique index: ${indexKey}`);
            }
    ...
    async dropIndex(indexName) {
        ...
        // Remove index file
        const indexPath = path.join(this.databaseDir, 'indexes', `${indexName}.idx`);
        try {
    ...
    async updateData(tableName, filter, updateData, transactionId = null) {
        if (!transactionId) {
            ...
            try {
                const result = await this.updateData(tableName, filter, updateData, transactionId);
                await this.commitTransaction(transactionId);
                return result;
            } catch (error) {
                await this.rollbackTransaction(transactionId);
        ...
        for (const [indexName, index] of this.indexes) {
            if (index.tableName === tableName) {
                ...
                if (oldKey !== newKey) {
                    ...
                    if (index.unique && index.data.has(newKey) &&
    ...
    async _getDataWithIndex(tableName, filter) {
        ...
        for (const [indexName, index] of this.indexes) {
            if (index.tableName === tableName) {
                const condition = filter.conditions.find(c =>
                    index.columns.includes(c.column) && c.operator === '=='
                );
    ...
    _applyFilter(data, filter) {
        ...
        return data.filter(item => {
            if (operator === 'and') {
                return conditions.every(condition => this._evaluateCondition(item, condition));
            } else if (operator === 'or') {
                return conditions.some(condition => this._evaluateCondition(item, condition));
            }
            return true;
        });
    }
    ...
    _evaluateCondition(item, condition) {
        ...
        switch (operator) {
            ...
            case 'like': return new RegExp(value, 'i').test(itemValue);
            case 'contains': return new RegExp(value).test(itemValue);
    ...
    async _acquireLock(tableName, transactionId, timeout = null) {
        ...
        return new Promise((resolve, reject) => {
            const attemptLock = () => {
                ...
                this.lockQueue.get(tableName).push({ transactionId, resolve, reject, startTime, timeout: lockTimeout });
            };
    ...
    async getStats() {
        return {
            ...
            lockQueue: Array.from(this.lockQueue.values()).reduce((sum, queue) => sum + queue.length, 0)
        };
    }
...
function createSecureDatabase(databaseDir, encryptionKey = null, options = {}) {
    return new SlimCryptDB(databaseDir, encryptionKey || generateEncryptionKey(), {
        encrypt: true,
        compression: true,
        walEnabled: true,
        syncWrites: true,
...
module.exports = { slimDB: SlimCryptDB, generateEncryptionKey, createSecureDatabase };
